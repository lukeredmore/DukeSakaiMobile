...
struct AsyncContentView<Item, Content: View>: View {
    ...
    init(loader: @escaping (_ collection: CourseCollection) async throws -> Item,
         content: @escaping (_ item: Item) -> Content,
        ...
        case .success(let item), .refreshing(let item):
            content(item)
                ...
                .onChange(of: env.selectedCollection) { col in
                    // This will reload info whether or not view is shown, which I think is a desireable behavior for speed (TODO: give visible view priority)
                    Task { await load(col)}
                }
